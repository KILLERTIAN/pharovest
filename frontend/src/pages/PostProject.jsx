...
import { createProjectOnBlockchain, getContract } from "@/utils/blockchainUtils";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!signer || !signer.provider) {
        ...
        await connectWallet();
        // Wait for a moment to ensure wallet connection is complete
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        // Check if we have a signer now
        if (!signer) {
          throw new Error("Failed to connect wallet. Please try connecting manually first.");
        }
      }
      
      const ethToUsdRate = 2410.28;
      
      // Make sure we have a properly connected signer before proceeding
      if (!signer || !signer.provider) {
        throw new Error("Wallet connection failed. Please try reconnecting your wallet.");
      }
      ...
      const blockchainMilestones = formData.milestones.map((milestone) => ({
        ...
        amountRequired: ethers.utils.parseEther((milestone.amountRequired / ethToUsdRate).toString()),
        recipient: '0xA5150981807ac926636be5713EF62A21DDC86f8C',
        isCompleted: false,
      }));

      // First create the project on the blockchain
      ...
      // Get the project ID from the blockchain (it should be the latest project ID)
      const contract = await getContract(signer);
      const projectCount = await contract.projectCount();
      const projectId = (projectCount - 1).toString();
      console.log(`Retrieved project ID from blockchain: ${projectId}`);
      
      // Now submit to the database with the blockchain project ID
      console.log("Submitting project to database with blockchain ID:", projectId);
      const data = new FormData();
      data.append("title", formData.title);
      data.append("description", formData.description);
      data.append("creator", storedUser?.user?.name || 'Anonymous'); 
      ...
      // Ensure projectId is a proper numeric string
      const numericId = parseInt(projectId, 10);
      if (isNaN(numericId)) {
        throw new Error("Failed to get a valid numeric project ID from blockchain");
      }
      
      // Convert to string and add to FormData
      data.append("id", numericId.toString());
      data.append("blockchainHash", transactionHash); // Include transaction hash directly
      
      if (formData.image) {
        data.append("image", formData.image);
      }
      
      const backendMilestones = formData.milestones.map((milestone) => ({
        title: milestone.title,
        description: milestone.description,
        completionDate: milestone.completionDate,
        amountRequired: milestone.amountRequired,
        upvotes: 0,
        downvotes: 0,
      }));
      ...
      try {
        ...
        // Navigate to projects page without showing any alert
        navigate("/projects");
      } catch (backendError) {
        console.error("Error saving project to backend:", backendError);
        
        // More detailed error for the user
        let errorMessage = backendError.message || "Unknown error";
        ...
        // If backend fails but blockchain succeeded, show more specific message
        alert(`Project was created on blockchain but failed to save to database: ${errorMessage}`);
      }
    } catch (error) {
